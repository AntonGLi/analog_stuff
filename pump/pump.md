...
В наличии у нас напряжения Vdd, Vclk, Vdd+Vclk, 0 (земля). 

В режиме зарядки промежуточной ёмкости левый (Т1) открыт, правый (Т2) закрыт: Vin=Vdd, $V_{bounce}\approx V_{dd}$, $Vout \approx Vclk+Vdd$. Если T1 PMOS, то на затвор можно подать имеющееся напряжение Vdd+Vclk. Если поставить NMOS, закрыть его можно напряжением Vdd или меньше.
