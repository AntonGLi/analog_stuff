...
Состояния инвертора можно изложить в таблице истинности.
...
Такой инвертор может работать как последовательное соединение инвертора и проходного ключа. Действительно, при подаче "неправильного" питания на одну из линий инвертор перестаёт выдавать этот логический уровень на выход. Пусть теперь $C=\overline{B}$ (то есть, входом $B$ мы теперь можем менять линии питания местами).
...
Это логика пропускания на выход инвертированного сигнала $A$.

$F=(B)\space?\space\overline{A}\space:\space Z$

Тогда схема эквивалентна инвертору с проходным ключом.
...
D-защёлку можно реализовать через мультиплексор, эта схема довольно распространена в интернете:

![](../media/d_latch.svg)

Теперь у нас есть компактная реализация мультиплексора на проходных ключах, и можно её применить для реализации триггеров.

Поскольку сами проходные ключи потенциал не создают, необходимо в цепь обратной связи добавить буфер. Тогда схема защёлки будет такой:

<img src="../media/d_latch_trans.svg" style="width: 85%"/>

Здесь 10 транзисторов (8, если инвертированный тактовый сигнал считать данным). По master-slave сразу можно сделать D-ff:

<img src="../media/d_ff_trans.svg" style="width: 100%"/>
